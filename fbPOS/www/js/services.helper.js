...
.factory('$helper', function($ionicLoading, $ionicPopup, $rootScope, $localStorage, $cordovaFileTransfer, $cordovaPrinter, $cordovaToast, $state, $ionicHistory, $ionicModal, $cordovaBarcodeScanner, $ionicSlideBoxDelegate, $ionicScrollDelegate, $q, $cordovaNetwork, $filter, $cordovaFile) {
    ...
    **************************************************/
    helper.toast = function(messsage, duration, location) {
        ...
        if (!ionic.Platform.isWebView()) {
            ...
            if ($rootScope.platform && $rootScope.platform != 'web') {
    ...
    **************************************************/
    helper.navBack = function(backCount, animation) {
        console.log('call helper.navBack()');
    ...
    **************************************************/
    ...
    helper.judgeUpdate = function(newStamp) {
        ...
        if ($rootScope.sandboxCheckbox) {
            oldUpdateTime = $localStorage.get('sandbox_update_time') != null ? $localStorage.get('sandbox_update_time') : '';
            $rootScope.sandboxUpdateTime = newStamp;
            if (oldUpdateTime != newStamp) {
                $rootScope.downloadData('UPDATE_DOWNLOAD_ASK');
            }
        } else {
            oldUpdateTime = $localStorage.get('production_update_time') != null ? $localStorage.get('production_update_time') : '';
            $rootScope.productionUpdateTime = newStamp;
            if (oldUpdateTime != newStamp) {
                $rootScope.downloadData('UPDATE_DOWNLOAD_ASK');
            }
        }
        ...
        console.log('old stamp: ' + oldUpdateTime + ' , new stamp: ' + newStamp);
    };
    /**************************************************
    // Return service
    **************************************************/
